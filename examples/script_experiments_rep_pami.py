...
def run_exp(experiment, snr,superv=False,mpm=True):
...
#sig = np.linalg.norm(mu)/np.sqrt(pargibbs.W)* 10**(-SNR/20.)
    ...
    sigma = 0.1
    sig = np.array([sigma,sigma]) ;     m = 0
    rho_1 = 0.5 * sig**2    ; rho_2 = 0.25 * sig**2
    X,V,Y = gen_exp(experiment,x_range,S0,S1,W,m,sig,rho_1,rho_2)
    
    
    
    x_mean = np.mean(X.astype(float))
    sigma = x_mean * 10**(-snr/20.)
...
#    parseg.seuil_conv = 0.05
    ...
    print 'SNR = %.0f dB : taux erreur HMF : %.7f, taux erreur TMF : %.7f'%(snr,Ex_hmf*100,Ex_tmf*100)   
    if experiment=='2':
...
#%%
for numexp in range(20,40):
    
    
   
    for superv in (False,True):
        if superv: n3 = 'known'; 
        else: n3='unknown'
...
#                else: n2='10'
                ...
                for experiment in ('2','3'):#('2','3'):
                    ...
                    try:
                        Y, X_mpm_est,V_mpm_est,Ux_map,Uv_map, parsem, X_mpm_hmf,Ux_hmf, parsem_hmf = run_exp(experiment,snr,superv,mpm)
