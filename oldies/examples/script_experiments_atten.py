...
def cast_angles(V,v_range):
    ...
    for i in range(V.shape[0]):
        for j in range(V.shape[1]):
            if np.isnan(V[i,j])==0:
                ecart = np.abs(V[i,j] - v_range_new)
                ind_min = np.argmin(ecart)
        
                V_new[i,j] =v_range_new[ind_min]   
    return V_new
#%%
...
#pi[0,:] = np.array([1e-01,   1e-04,   1e-04,1e-04,  1e-04,   1e-04, 1e-04,   1e-02,   9e-01])  
#pi[1,:] = np.exp(np.arange(9))
...
V =  cast_angles(zoom(dat['V'], (ratio,ratio),order=0),v_range)
...
nb_rea = 100 # il faudrait 100 ici, pour estimer des frequences proprement
taille_fen = 5
seuil_conv = 0.02
pargibbs.Xi = 0.1
##
...
pargibbs.thr_conv = 1e-3
pargibbs.v_help = True
nb_iter_mpm = 150
pargibbs.nb_iter = 100 
        
SNR_atten = np.array([-20,-10,-5])
...
X_mpm_att,V_mpm_att, parsem = sot.seg_otmf_atten(pargibbs,
                                                             nb_iter_sem,
                                                             nb_iter_mpm,
                                                             nb_rea,seuil_conv,
                                                             taille_fen,longueur_mpm=1,
                                                             snr_att=SNR_atten,
                                                             incert=incert
                                                             )

#    X_mpm_hierarch[:,:,level] =  zoom(X_mpm_att, (1./zoom_ratio,1./zoom_ratio),order=0)
#    V_mpm_hierarch[:,:,level] =  zoom(V_mpm_att, (1./zoom_ratio,1./zoom_ratio),order=0)
#    print '-----'
## 
end = time.time() - start
...
levels_att = np.sqrt(pargibbs.W)*pargibbs.sig/np.linalg.norm(pargibbs.mu) * 10**(SNR_atten/20.)     
...
plt.axis('off')
plt.title('$\mathbf{y}^0$, moyenne spectrale')

plt.subplot(nb_li,nb_col,2)
plt.imshow(X, interpolation='nearest', origin='lower', cmap=plt.cm.gray); 
X_aux = np.zeros_like(X)

X_aux[X<=min(levels_att)] = 0
for l in range(levels_att.size):
    X_aux[(X>levels_att[l])* (X<levels_att[l+1])] =l+1

X_aux[X>max(levels_att)] = l+1
la_aux = np.arange(SNR_atten.size+1)
cs=plt.contourf(X_aux,alpha=0.5,labels=str(SNR_atten),antialiased=False,cmap=plt.cm.jet)
plt.axis('off')
plt.title('$\mathbf{x}$ et  $I_{\mathrm{SNR}} \in \lbrace -20, -10,-5 \\rbrace \mathrm{dB}$')
plt.xlim((0.5,S0-1.5))
plt.ylim((0.5,S1-1.5))
...
plt.imshow(X, interpolation='nearest', origin='lower', cmap=plt.cm.gray); 
plt.imshow(V, interpolation='nearest', origin='lower', cmap=plt.cm.Spectral,vmin=0,vmax=np.pi, alpha=0.75); 
...
plt.imshow(Y.mean(axis=2), interpolation='nearest', origin='lower', cmap=plt.cm.gray,); 
plt.contourf(X_mpm_att.sum(axis=2),level=np.arange(SNR_atten.size+1),alpha=0.5,labels=SNR_atten,antialiased=False,cmap=plt.cm.jet)
plt.axis('off')
plt.title('$\\hat{\mathbf{x}}_{\mathrm{MPM}}$ selon $I_{\mathrm{SNR}} \in \lbrace -20, -10,-5 \\rbrace \mathrm{dB}$')
plt.xlim((0.5,S0-1.5))
plt.ylim((0.5,S1-1.5))
...
plt.imshow(Y.mean(axis=2), interpolation='nearest', origin='lower', cmap=plt.cm.gray,); 
plt.contourf(V_mpm_att.mean(axis=2),alpha=0.5,cmap=plt.cm.Spectral,vmin=0,vmax=np.pi)
plt.axis('off')
plt.title('$\\hat{\mathbf{v}}_{\mathrm{MPM}}$ selon $I_{\mathrm{SNR}} \in \lbrace -20, -10,-5 \\rbrace \mathrm{dB}$')
plt.xlim((0.5,S0-1.5))
plt.ylim((0.5,S1-1.5))
...
###############################################################################
#
#for level in range(levels_atten.size):
#    
#    plt.subplot(nb_li,nb_col,nb_col*(nb_li-level-1)+2)
#    plt.imshow(X_mpm_att[:,:,level], interpolation='nearest', origin='lower', cmap=cm_gris,vmin=0,vmax=1); 
#    #    plt.contour(V_mpm_att[:,:,level]==v_range[0], 1,colors='g',linewidths=5,alpha=0.75)
#    plt.axis('off')
...
#    plt.imshow(V_mpm_att[:,:,level], interpolation='nearest', origin='lower',cmap=cm_angl,vmin=0,vmax=np.pi); 
#    plot_directions(V_mpm_att[:,:,level], np.ones_like(V_mpm_att[:,:,level]),pas=8)
#    plt.axis('off')
...
#plt.subplot(131)
#plt.imshow(an, interpolation='nearest', origin='lower',cmap=cm_angl,vmin=0,vmax=np.pi); 
#plot_directions(an, np.ones_like(V_mpm_hierarch[:,:,level]),pas=8)
#plt.axis('off')
#
#plt.subplot(132)
#plt.imshow(an_interp, interpolation='nearest', origin='lower',cmap=cm_angl,vmin=0,vmax=np.pi); 
